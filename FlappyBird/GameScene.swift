...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func setupCloud() {
        ...
        // スクロールするアクションを作成
        // 左方向に画像一枚分スクロールさせるアクション
        let moveCloud = SKAction.moveBy(x: -cloudTexture.size().width , y: 0, duration: 10.0)
    ...
    func setupWall() {
        ...
        // 画面外まで移動するアクションを作成
        let moveWall = SKAction.moveBy(x: -movingDistance, y: 0, duration:1.0)
        ...
        let createWallAnimation = SKAction.run({
            ...
            item.position = CGPoint(x: 0.0, y: under_wall_y + wallTexture.size().height - slit_length * 1 / 3)
        ...
        let waitAnimation = SKAction.wait(forDuration: 0.4)
